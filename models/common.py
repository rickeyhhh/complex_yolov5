...
class QuantizedConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias):
        ...
        self.reset_complex_parameters()

    def reset_complex_parameters(self):
...
class ComplexConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        """Initializes a complex-valued convolution layer with optional batch normalization and activation."""
        super().__init__()
        self.conv = QuantizedConv2d(c1, c2, k, s, autopad(k, p, d), dilation=d, groups=g, bias=False)
    ...
    def forward(self, x):
        ...
        y_real_conv, y_imag_conv = self.conv(x_real, x_imag)

        y_real = self.bn_real(y_real_conv)
...
class ComplexBottleneck(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = ComplexConv(c1, c_, 1, 1)
        self.cv2 = ComplexConv(c_, c2, 3, 1, 1, g=g)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        # print("x.shape", x.shape)
        # y1 = self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
...
class ComplexC3(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)
        self.cv1 = ComplexConv(c1, c_, 1, 1)
        self.cv2 = ComplexConv(c1, c_, 1, 1)
        self.cv3 = ComplexConv(2 * c_, c2, 1)
        self.m = nn.Sequential(*(ComplexBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
...
class MagnitudeMaxPool2d(nn.Module):
    def __init__(self, k, s, p):
        super().__init__()
        self.m = nn.MaxPool2d(kernel_size=k, stride=s, padding=p, return_indices=True)
...
class ComplexSPPF(nn.Module):
    def __init__(self, c1, c2, k=5):
        ...
        c_ = c1 // 2  # hidden channels
        self.cv1 = ComplexConv(c1, c_, 1, 1)
        self.cv2 = ComplexConv(c_ * 4, c2, 1, 1)
        self.m = MagnitudeMaxPool2d(k, 1, k // 2)
